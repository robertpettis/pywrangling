...
def apply_beamer_theme(
    ...
    *,
    ...
    Parameters
    ----------
    cell_tags : bool, default False
        If *True*, process cell-level tags after applying the theme.
        Recognised tags:

        *Always active* (notebook + slideshow):

        * ``remove_input``  — hide the code-input area of the cell.
        * ``remove_output`` — hide the output area of the cell.
        * ``side_by_side``  — display input and output side by side.
        * ``hide_prompt``   — hide the In[N]/Out[N] prompt labels.

        *Slide-only* (cells stay fully visible in notebook editing):
    ...
    notebook_path : str or Path, optional
        Notebook to read cell tags from.  Required when *cell_tags* is True
        and auto-detection does not work.

    lock_markdown : bool, default False
        If *True*, prevent editing markdown cells by double-clicking during
        slideshow mode.  This keeps the presentation clean by preventing
        accidental entry into edit mode.  Normal notebook editing is unaffected.
...
/* ===== Background bars for header/footer ===== */
/* Real DOM elements on document.body — immune to Reveal.js transforms. */
/* Themed backgrounds are set per-theme (replacing old ::before/::after). */
.beamer-head-bg {
  position: fixed;
  top: 0; left: 0;
...
/* Kill the old pseudo-element bars — replaced by .beamer-head/foot-bg */
.reveal::before,
.reveal::after {
...
/* Only hide the in-slide heading AFTER JS confirms header is alive */
body.beamer-shell-ready .reveal .slides section.present h2 {
  display: none;
}
...
"""
    ...
    if theme_key == "stanford":
        return r"""
/* Stanford theme – outer: infolines, inner: rounded, colors: cardinal red */
:root {
  --bm-cardinal:       #8C1515;
  --bm-cardinal-dark:  #820000;
  --bm-cardinal-light: #B83A4B;
  --bm-white:          #FFFFFF;
  --bm-black:          #2E2D29;
  --bm-cool-gray:      #53565A;
  --bm-structure:      #8C1515;
  ...
  --bm-accent-green:      #175E54;
  --bm-sandstone:      #D2C295;

  --bm-block-bg:         #8C1515;
  --bm-block-body-bg:    #F4E3E3;
  --bm-alert-bg:         #820000;
  --bm-alert-body-bg:    #F2DEDE;
  --bm-example-bg:       #175E54;
  --bm-example-body-bg:  #DEF0ED;
}

/* ── Header bar (Stanford stripes: cardinal, white, black, cool gray) ── */
.beamer-head-bg {
  top: 0; left: 0;
  width: 100%; height: 40px;
  background: linear-gradient(
...
/* ── Footer bar (Stanford stripes matching header) ── */
.beamer-foot-bg {
  bottom: 0; left: 0;
  width: 100%; height: 28px;
  background: linear-gradient(
...
/* ── Headline — larger font, positioned over cardinal stripe ── */
.beamer-headline {
  color: #fff;
  font-size: 22px;
}
.beamer-headline .beamer-head-left {
  /* Logo area over cardinal stripe */
  width: 25%;
  max-width: 25%;
  flex: 0 0 25%;
}
.beamer-headline .beamer-head-title {
  /* Title over white stripe — use cardinal text */
  color: var(--bm-cardinal);
  text-shadow: none;
  font-size: 20px;
}

/* ── Footline — larger font ── */
.beamer-footline {
  font-size: 14px;
}
.beamer-footline .beamer-foot-left {
  /* Author over cardinal stripe — white text */
  color: #fff;
  flex: 0 0 25%;
  max-width: 25%;
}
.beamer-footline .beamer-foot-center {
  /* Date over white stripe — cardinal text */
  color: var(--bm-cardinal);
  flex: 0 0 25%;
  max-width: 25%;
}
.beamer-footline .beamer-foot-right {
  /* Slide number over black/gray stripes — white text */
  color: #fff;
  flex: 0 0 50%;
  max-width: 50%;
}
...
/* ── Slide number badge ── */
.reveal .slide-number {
  background: var(--bm-cardinal-dark);
  color: #fff;
  bottom: 0; right: 8px;
  z-index: 1002;
}
...
/* ── Blocks use accent green for examples ── */
.beamer-block.example .block-title {
  background: var(--bm-accent-green);
}
"""
    ...
    if theme_key == "gamecocks":
        return r"""
/* Gamecocks theme – University of South Carolina official colors */
:root {
  --bm-garnet:         #73000A;
  --bm-garnet-dark:    #5C0008;
  --bm-garnet-light:   #8C1919;
  --bm-black:          #000000;
  --bm-white:          #FFFFFF;
  --bm-gold:           #FFD983;
  --bm-structure:      #73000A;
  ...
  --bm-block-bg:         #73000A;
  --bm-block-body-bg:    #F4E3E4;
  --bm-alert-bg:         #5C0008;
  --bm-alert-body-bg:    #F2DEDE;
  --bm-example-bg:       #2E5A35;
  --bm-example-body-bg:  #DEF0DE;
}

/* ── Header bar (garnet/black/gold stripes) ── */
.beamer-head-bg {
  top: 0; left: 0;
  width: 100%; height: 40px;
  background: linear-gradient(
    to right,
    var(--bm-garnet)  0%,  var(--bm-garnet)  40%,
    var(--bm-black)   40%, var(--bm-black)   70%,
    var(--bm-gold)    70%, var(--bm-gold)    100%
  );
  z-index: 1000;
}

/* ── Footer bar (matching stripes) ── */
.beamer-foot-bg {
  bottom: 0; left: 0;
  width: 100%; height: 28px;
  background: linear-gradient(
...
def _write_nbconvert_template(assets_root: Path, theme_key: str) -> Path:
    ...
    # --- Build the full CSS (global first, then theme-specific to override) ---
    full_css = _GLOBAL_BEAMER_CSS + "\n" + _theme_css(theme_key)
